...
class Argument:
    def __init__(self,name,itype="Object"):
        self.name = name
        self.itype = itype
        self.calls = []
    def setItype(itype):
...
class ClassInfo:
    ...
    def __str__(self):
        ...
        for m in self.methods:
            methods += m.__str__() + "\n"
        return "Printing ClassInfo...\n" + \
               "Constructor: " + self.constructor.__str__() + \
               "\n-----------\n" + \
               "Methods:" + "\n-------\n" + methods
...
class ProgramInfo:
    ...
    def __str__(self):
        ...
        for c in self.classes:
            classes += c.__str__() + "\n"
        return "=== PRINTING PROGRAM INFO ===\n\n" + classes

def seekMethod (node):
    if hasattr(node,'expression'):
        if hasattr(node.expression,'type'):
            if node.expression.type == "ASSIGN":
                lhs = node.expression[0]
                rhs = node.expression[1]
                if rhs.type == "FUNCTION" and rhs.value == "function":
                    if rhs.functionForm == 1:
                        if lhs.type == "DOT":
                            if lhs[0].type == "THIS":
                                methodInfo = MethodInfo(lhs[1].value,rhs.params)
                                return methodInfo

if __name__ == "__main__":
    #buildClassInfo(js.parse(file(sys.argv[1]).read(),sys.argv[1]),classInfo),
    try:
        include_props = (sys.argv[1] == "--props")
    except:
        include_props = False
    ast = js.parse(file(sys.argv[1]).read(),sys.argv[1])
    
    # making sure we are dealing with the outermost node of the AST
    assert ast.type == "SCRIPT"
    assert ast.lineno == 1
    assert ast.funDecls
    _classes = ast.funDecls
    classes = []
    for c in _classes:
        assert c.type == "FUNCTION"
        assert c.functionForm == 0
        assert c.value == "function"
        constructor = MethodInfo(c.name,c.params)
        methods = []
        for node in c.body:
            m_info = seekMethod(node)
            if m_info is not None:
                methods.append(m_info)
        classInfo = ClassInfo(constructor,methods)
        classes.append(classInfo)
    programInfo = ProgramInfo(classes)
    print programInfo
