...
var firstparse = function (expr) {
    if (typeof expr !== 'string') throw 'expression should be a string';
    
    try {
        var ast = parser.firstparse.apply(null, arguments);
    }
    catch (err) {
...
var burrito = module.exports = function (code, cb, names) {
    var ast = Array_isArray(code)
        ? code // already an ast
        : parse(code.toString(), false, true);

//  console.log(util.inspect(traverse(ast), false, null));
    var ast_ = traverse(ast).map(function mapper () {
        wrapNode(this, cb);
    });
    var nodes = traverse(ast_).nodes();
    var nodeIndexes = [];
    for (var i = 0; i<nodes.length; i++) {
        if (_.indexOf(_.values(names),nodes[i]) !== -1) {
            nodeIndexes.push(nodes[i+1][0][1]);
        }
    }

    /*forEach(function(x){
        if(_.indexOf(_.values(names),x) !== -1) {
            console.log(this.path);
        }
    })*/
    return {mut: deparse(parse(deparse(ast_)), true),
