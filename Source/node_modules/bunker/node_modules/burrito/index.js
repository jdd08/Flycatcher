...
var retdump = require('../../../../utils.js').retdump;
...
var burrito = module.exports = function (code, cut, mut, cb) {
    var ast = Array_isArray(code)
        ? code // already an ast
        : parse(code.toString(), false, true);
    //dump(ast);
    //fs.writeFileSync('ast',retdump(ast));
    var ast_ = traverse(ast).map(function mapper () {
        wrapNode(this, cb);
    });
    //fs.writeFileSync('ast_',retdump(ast_));
    
    /*dump(ast_);
    console.log()
    dump(deparse(ast_),true)
    console.log()
    dump(parse(deparse(ast_)));
    console.log()
    dump(deparse(parse(deparse(ast_))));*/
