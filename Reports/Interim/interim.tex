...
Symbolic execution gathers constraints along a simulated execution of a program path, where symbolic variables are used instead of actual values, such that the final path predicate can be rewritten in terms of the input variables. Solving the resulting system of constraints then yields the data necessary for the traversal of the given path \cite{king1976symbolic}. %ramamoorthy
However, this is easier said than done. Edvardsson outlines some difficulties such as a potentially unbounded number of execution paths in the presence of loops or dynamic structures such as pointers, arrays or objects %cite ramamoorthy and pasareanu
