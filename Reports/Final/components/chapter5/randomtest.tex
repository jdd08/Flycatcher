...
For all the results of calling the MUT in a candidate test to be recorded, so that the assertions seen in example \ref{final} can be created, the format of test \ref{final} in the \textbf{Executor} is in fact:
...
Although we have discussed how tests are constructed, we have purposely ignored one important detail: \emph{we do not know the types of any function parameters}. How do we know that the \texttt{LinkedList.add} method takes an object of type \texttt{Node}?
...
\begin{itemize}
   \item It achieved new coverage in the \textbf{Executor} (the current coverage is that achieved by other tests so far)
