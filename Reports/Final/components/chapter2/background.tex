...
% The choice of implementation for \textsf{Flycatcher} is dynamic random ATDG as it 
		...
		child [yshift=1cm] {
			...
			child {node [level 1,xshift=0.4cm] {Structural (white-box)}
				...
				child {node [level 2] {Dynamic}
		     	      ...
		     	      child {node [level 3,text width=2.5cm,xshift=-0.4cm] {\color{myblue}Random}}
				}
			}
		};
\end{tikzpicture}
...
As for real software errors, \textsf{Flycatcher} will deal with those too in a way that will be described in later chapters.
...
% Hence, this measure can be used, as for the generation of input \emph{data}, as a search objective to guide the generation of \emph{method call sequences} for our test cases.Tonella was one of the first to use search-based methods for the generation of adequate object-oriented test cases, using genetic algorithms as the search heuristics method \cite{tonella2004evolutionary}. Indeed, the procedure of using genetic algorithms to generate input test data for \textsf{Flycatcher} described in~\ref{subsubsec:ga_application} can simply be extended to the generation of object-oriented tests by adapting individuals so that they represent the structure of a test case.
...
It is worth noting about \emph{Proxies} that some operations are not trapped in order to respect the language invariants:
