		...
		child [yshift=1cm] {
			...
			child {node [level 1,xshift=0.4cm] {Structural (white-box)}
				...
				child {node [level 2] {Dynamic}
		     	      ...
		     	      child {node [level 3,text width=2.5cm,xshift=-0.4cm] {\color{blue}Random}}
				}
			}
		};
\end{tikzpicture}
...
% Below we describe the random and search-based approaches in more detail, as they are the approaches used in \textsf{Flycatcher}.
% TODO GENETIC
...
% TODO GENETIC and is considered to have the lowest acceptance rate \cite{edvardsson1999survey}, it forms the basis of the \textsf{Flycatcher} application.
...
\texttt{fix()} & \texttt{Object.freeze(proxy)} \\
                        & \texttt{Object.seal(proxy)} \\
                        & \texttt{Object.preventExtensions)} \\
\hline
\emph{Derived Traps} & \\
...
It is worth noting about Proxies, as this will matter to us later, that some operations are not trapped in order to respect the language invariants:
...
There is much more to say about object Proxies and their applications, but this brief introduction is sufficient in our case. The full proposal can be found online\footnote{"\url{http://wiki.ecmascript.org/doku.php?id=harmony:proxies}"}.
...
\textsf{Flycatcher} is the first tool to automatically generate unit-test suites for JavaScript programs written in an object-oriented style, where classes are the target unit. In this section, we discuss some similar work in different languages, as well as projects that have applied different techniques for the generation of tests in JavaScript.
